...
const [sourceOptions, setSourceOptions] = useState([]);
  ...
  const closeBulkEditDialog = () => {
    ...
    const fetchTotalCandidates = async () => {
        try {
            ...
            let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}job/jobApplications`;
           ...
           // If a search query is provided, add it to the API URL
            ...
            console.error("in data total",data.length);
  ...
  const fetchCandidates = async (page,searchQuery) => {
    try {
        ...
        let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}job/jobApplications?page=${page}`;
...
<table className="table table-striped table-bordered">
  ...
  <tr key={candidate._id}>
    {/* <td>
      <input
        type="checkbox"
        checked={selectedCandidates.includes(candidate?.candidate?._id)}
        onChange={() => toggleCandidateSelection(candidate?.candidate?._id)}
      />
    </td> */}
    <td>
      <Link href={`/admin-dash/edit-application/${candidate?.candidate?._id}`}>
        {`${candidate?.candidate?.name}`}
      </Link>
    </td>
    <td>{candidate?.candidate?.phone}</td>
    <td>{candidate?.candidate?.currentCity}</td>


    <td>{candidate?.job?.jobTitle}</td>
    {/* <td>{candidate?.job?.company?.name}</td>
    <td>{candidate?.candidate?.leadOwner}</td> */}
